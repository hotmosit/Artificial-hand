/*
 * This tab is for class FingerUp
 * this class draw up view of the finger.
 */


class FingerUp implements Finger {
  ...
  FingerUp(float fingerStartX, float fingerStartY, float fingerStartZ, float len ) {
    // set length
    this.len1 = len  ;
    this.len2 = len * 4/5;
    this.len3 = len * 4/5;   
  ...
  float j2gm(Button grab, int angle, Serial s) {
    if ((j1gm(grab, angle, s) > 0 && angle2 < angle1*2.1 && angle2 > -0.1)
         ||(flag == true && angle2 < angle1 * 4.3) ) {
      angle2 += radians(angleMov) ;

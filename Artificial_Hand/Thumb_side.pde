class ThumbSide implements Finger {
  ...
  ThumbSide( float start_x, float start_y, float start_z, float len) {
    // set length
    this.len1 = len;
    this.len2 = len * 3/2;
    this.len3 = len * 2;
    ...
    this.tip_x = start_x;
    // set joint Y cor
    this.joint_1_y = start_y;
    this.joint_2_y = start_y;
    this.tip_y = start_y;
    // set joint Z cor 
    this.joint_1_z = start_z + len1;
    this.joint_2_z = joint_1_z + len2;
    this.tip_z = joint_2_z + len3;  
    // set angles
    this.angle1 = 0;
    this.angle2 = 0;
    this.angle3 = 0;
  ...
  float j1gm(Button grab, int angle, Serial s) {
    if ((grab.activate == true && angle1 < PI/(-(angle)+8) && angle1 > -0.1 && activate == true)
      ...
      joint_1_z = start_z + (len1 * cos(-angle1));
  ...
  float j2gm(Button grab, int angle, Serial s) {
    if ( j1gm(grab, angle, s) > 0 && angle2 < angle1 * 2.1 && angle2 > -0.1) {
      ...
      joint_2_z = joint_1_z + (len2 * cos(-angle2));
  ...
  void tgm(Button grab, int angle, Serial s) {
    if (j2gm(grab, angle, s) > 0 && angle3 < angle2 * 3/2 && angle3 > -0.1) {
      ...
      tip_z = joint_2_z + (len3 * cos(-angle3));
